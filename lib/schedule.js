...
function emitClipBlock(entries, dayStart, slotStart, shortPool, shortCursor, used) {
    ...
    while (t < limit && count < CONFIG.slots.clipBlock.maxClips) {
        ...
        entries.push({ t: dayStart + t, v: v.individual_id, d: Math.round(dur), kind: 'show', title: v.collection_title });
...
function emitSingleWithAds(entries, dayStart, slotStart, show, adPool, adRng, used) {
    ...
    for (let i = 0; i < segCount; i++) {
        ...
        entries.push({ t: dayStart + t, v: show.individual_id, d: Math.round(thisSeg), kind: 'segment', seek: Math.round(resume), title: show.collection_title });
        t += thisSeg;
        resume += thisSeg;
        if (i < ads.length) {
            const ad = ads[i];
            const adShare = fillNeeded / ads.length;
            entries.push({ t: dayStart + t, v: ad.individual_id, d: Math.round(adShare), kind: 'ad', title: ad.collection_title });
            t += adShare;
        }
    }
    return true;
