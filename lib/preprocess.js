...
class ArchiveVideoFinder {
    ...
    getSearchQueries() {
        return [
            {
                category: 'glitch_art',
                query: '(glitch OR datamosh OR "data mosh" OR "pixel sorting" OR "digital corruption" OR "compression artifact" OR "video glitch" OR "corrupted video") AND (art OR video OR experimental) AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
                category: 'vj_loops',
                query: '("VJ loop" OR "vj loop" OR "video loop" OR "live visual" OR "motion graphics" OR "visual effect" OR "audiovisual") AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
                category: 'experimental_video',
                query: '(experimental OR surreal OR abstract OR "video art" OR psychedelic OR trippy OR "abstract animation") AND (video OR film OR art) AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
                category: 'internet_weirdness',
                query: '((creepypasta OR "analog horror" OR "internet horror" OR "weird internet" OR "deep web" OR cursed OR eerie) AND (video OR compilation OR footage)) AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
                category: 'absurdist_video',
                query: '(("tim and eric" OR "too many cooks" OR "everything is terrible" OR absurdist OR dada OR nonsense) AND (video OR compilation)) AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
                category: 'adult_swim',
                query: '("adult swim" OR "robot chicken" OR "moral orel" OR "superjail" OR "sealab 2021" OR "aqua teen hunger force" OR "metalocalypse" OR "squidbillies" OR "12 oz mouse" OR "xavier renegade angel") AND mediatype:(movies OR opensource_movies)',
                filters: [],
                sort: 'downloads desc'
            },
            {
    ...
    async findAllVideos(callback) {
        const knownCollections = [
            'SoundtrackToMadness', 'OperationSundevil', 'MemoryLoss',
            'TooManyCooks', 'EverythingIsTerribleArchive', 'GlitchArtCompilation',
            'VJLoopArchive', 'CorruptedMedia', 'DatamoshArt',
            'TimAndEricAwesomeShow', 'XavierRenegadeAngel', 'MoralOrel',
            'RobotChicken', 'Superjail', 'DigitalCorruption'
        ];

        console.log(`\n🔍 Using ${knownCollections.length} known collections`);

        for (const identifier of knownCollections) {
            try {
                const collection = await this.processCollection(identifier);
                if (collection) {
                    callback(collection);
                    console.log(`   ✓ ${identifier}`);
                } else {
                    console.log(`   ✗ ${identifier} (filtered out)`);
                }
                await new Promise(resolve => setTimeout(resolve, CONFIG.web.delay));
            } catch (error) {
                console.warn(`   Error processing ${identifier}: ${error.message}`);
            }
        }

        console.log(`\n🔍 Fallback: Searching archive.org...`);
...
async function process() {
    ...
    processed.unique_collections = processed.videos.length;

    // Multiply collections to reach minimum volume for 52 weeks
    // 168 slots/week * 52 weeks = 8736 slots needed
    // Average ~40 videos per collection = 218 collections needed
    const minCollectionsFor52Weeks = 200;
    while (processed.videos.length < minCollectionsFor52Weeks && processed.unique_collections > 0) {
        const originalLength = processed.unique_collections;
        for (let i = 0; i < originalLength && processed.videos.length < minCollectionsFor52Weeks; i++) {
            const original = processed.videos[i];
            const cloned = JSON.parse(JSON.stringify(original));
            cloned.identifier = `${original.identifier}_copy_${Math.floor(processed.videos.length / originalLength)}`;
            processed.videos.push(cloned);
        }
    }

    console.log(`\n📊 Total collections: ${processed.videos.length}`);
    console.log(`📊 Unique collections: ${processed.unique_collections}`);
    return processed;
}
